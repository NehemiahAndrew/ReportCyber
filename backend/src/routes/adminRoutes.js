...
router.get('/dashboard', async (req, res, next) => {
  try {
    const [
      ...
      Report.countDocuments({ 'verification.isVerified': true }),
      User.aggregate([
        { $match: { isActive: true } },
      ...
      Report.find()
        .sort({ createdAt: -1 })
        .limit(5)
        .select('reportId title incidentType status createdAt'),
      User.find()
        .sort({ createdAt: -1 })
        .limit(5)
...
router.patch('/users/:id/role', async (req, res, next) => {
  try {
    const { role } = req.body;
    const validRoles = ['user', 'analyst', 'moderator', 'admin'];
